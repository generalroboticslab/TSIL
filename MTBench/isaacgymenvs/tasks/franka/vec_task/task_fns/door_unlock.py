...
def create_envs(
    ...
    self.door_unlock_dof_lower_limits = to_torch(door_dof_lower_limits, device=self.device)
    self.door_unlock_dof_upper_limits = to_torch(door_dof_upper_limits, device=self.device)
...
def compute_reward(
    ...
    rewards = (2 * ready_to_push + 8 * pushed)
...
def reset_env(env,tid, env_ids, random_reset_space):
    ...
    # reset door to LOCKED (door_unlock task starts with locked door)
    ...
    all_pos[...,0][door_dof_idxs_long] = self.door_unlock_dof_upper_limits
