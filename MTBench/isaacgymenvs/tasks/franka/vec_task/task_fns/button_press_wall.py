...
def create_envs(
    ...
    self._button_press_wall_offset = torch.tensor([0.0, 0.0, 0.075], device=self.device, dtype=torch.float32)
...
def compute_observations(env, env_ids):
    ...
    wall_pos = self._root_state[self.franka_actor_idx[env_ids]+2,:3] + self._button_press_wall_offset
    wall_quat = self._root_state[self.franka_actor_idx[env_ids]+2,3:7]
...
def reset_env(env,tid, env_ids, random_reset_space):
    ...
    self._root_state[wall_multi_env_ids_int32,0] = buttonbox_x - .30
    self._root_state[wall_multi_env_ids_int32,1] = buttonbox_y - .05
