...
def compute_observations(env, env_ids):
    ...
    midpoint = (self.obj_init_pos[env_ids, :3] + self.target_pos[env_ids, :3]) / 2
    # self.specialized_kwargs['pick_place_wall']['midpoint'][:,2] = (self.target_pos[0,2] - self.table_surface_pos[2]) / 2 + self.table_surface_pos[2]
    midpoint[:, 0] = .17
    # self.specialized_kwargs['pick_place_wall']['midpoint'][1] += .075
    midpoint[:, 2] = self.target_pos[env_ids, 2]
    self.specialized_kwargs['pick_place_wall']['midpoint'] = midpoint
...
def compute_reward(
    ...
    obj_to_target_success = torch.norm(obj - target_pos,dim=-1)
    ...
    success = obj_to_target_success <= SUCCESS_RADIUS
