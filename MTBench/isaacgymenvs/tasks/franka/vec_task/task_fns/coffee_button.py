...
def compute_reward(
    ...
    rewards = torch.where((tcp_to_obj <= 0.05) & (tcp_closed>=.95), rewards + 8*button_pressed, rewards)
...
def reset_env(env,tid, env_ids, random_reset_space):
    ...
    self._root_state[coffee_machine_multi_env_ids_int32,:3] = self.last_rand_vecs[env_ids,:3]

    mug_multi_env_ids_int32 = (self.franka_actor_idx[env_ids]+2).flatten().to(dtype=torch.int32)
    self._root_state[mug_multi_env_ids_int32,:3] = self.obj_init_pos[env_ids] + to_torch([-.15,0,0],device=self.device)
    self._root_state[mug_multi_env_ids_int32,2] = self._table_surface_pos[2] + .005  
